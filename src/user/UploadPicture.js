...
const UploadPicture = (props) => {
    ...
    const local = null//"http://localhost:3002"
    ...
    const onChange = async (e) => {
        ...
        const res = await axios.post(`${local || process.env.REACT_APP_IP}/api/users/picture`, formData, {
            headers: {
    ...
    const onClick = async () => {
        ...
        if (!mail || !nickName || !password || !passwordCheck) {
            ...
            const data = {
                ...
                const res = await axios.post(`${local || process.env.REACT_APP_IP}/api/users/register`, data)
                if(res.status === 200){
                    alert("회원가입이 완료되었습니다.")
                    navigate(-1)
                }else{
        ...
        <>
            <div className="user">
                ...
                <div className="profile-img">
                    {upImg ? <img src={process.env.REACT_APP_IP + upImg} alt=""/> :
